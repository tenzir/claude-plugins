...
An Incident Finding reports the creation, update, or closure of security incidents as a result of detections and/or analytics.  <strong>Note: </strong>`Incident Finding` implicitly includes the `incident` profile and it should be added to the `metadata.profiles[]` array.
...
| `class_uid` | integer_t |
| `cloud` | object (cloud) |
| `finding_info_list` | object (finding_info) |
